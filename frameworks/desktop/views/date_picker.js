  ...
  mouseUp: function(evt) {
    ...
    if (className.match('button')) {
      ...
      this.set('displayFromDate', displayFromDate.advance(param));
			console.log("displayFromDate (before rendering)",displayFromDate.advance(param));
  ...
  */
  render: function(context, firstTime) {
    var displayFromDate = this.get('displayFromDate'),
		...
		if (this.get('firstDayOfTheWeek') === 'Monday') {
			if (startDay === 0) {
				startDay = 6; 
			} else {
				--startDay;
    ...
    var currDate = displayFromDate.advance({day: -startDay}), 
		...
		classNames, uniqueDayIdentifier, isCurrentMonth, isActiveDate,
		isToday, isSelectedDate, isBeingSelectedDate; // MC
    ...
    context = context.begin('div').addClass('body');
		...
		// Monday or Sunday as first day of week
		if (this.get('firstDayOfTheWeek') === 'Monday') {
			// Monday as first day of week
	    for (var i = 1; i < 8; i++) {
				if (i<7) {
					...
					context.begin('div').addClass('day name').text(weekdayStrings[0]).end();
				}
	    }
		} else {
			// Sunday as first day of week
	    for (var j = 0; j < 7; j++) {
	      context.begin('div').addClass('day name').text(weekdayStrings[j]).end();
	    }		
		}
    ...
    for (var gIdx = 0; gIdx < 42; gIdx++) {
      ...
      isBeingSelectedDate = mouseDownDate && currDate.get('day') === mouseDownDate.get('day') && currDate.get('month') === mouseDownDate.get('month') && currDate.get('year') === mouseDownDate.get('year');
			...
			console.log(isToday, isSelectedDate, isBeingSelectedDate);
